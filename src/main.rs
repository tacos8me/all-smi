...
async fn run_command(cli: Cli, settings: Settings) {
    ...
    match cli.command {
        ...
        Some(Commands::View(mut args)) => {
            ...
            args.icculis |= settings.consolidated.icculis;
            if args.icculis_health.is_none() {
                args.icculis_health = settings.consolidated.icculis_health_url.clone();
            }
            if args.icculis_swap.is_none() {
                args.icculis_swap = settings.consolidated.icculis_swap_url.clone();
            }
            // The pipeline panel lives in the Consolidated tab.
            args.consolidated |= settings.consolidated.enabled || args.icculis;
